...
# FFmpeg Service - Build and Push Script
# Usage: ./build-external.sh [build|push|build-push|login] [web|all]
...
get_harbor_credentials() {
    ...
    # 1. Registry Domain Name
    if [ -n "$REGISTRY_DOMAIN_NAME" ] && [ -n "$HARBOR_USERNAME" ] && [ -n "$HARBOR_PASSWORD" ]; then
        print_info "Using credentials and registry from environment variables"
        return 0
    fi

    # Prompt for registry domain if not set
    if [ -z "$REGISTRY_DOMAIN_NAME" ]; then
        echo -n "Enter Harbor registry domain: "
        read REGISTRY_DOMAIN_NAME
        if [ -z "$REGISTRY_DOMAIN_NAME" ]; then
            print_error "Registry domain cannot be empty"
            exit 1
        fi
...
# Function to build Docker image for FFmpeg service
build_image() {
    local service="$1"
    ...
    case "$service" in
        web)
            # Script is in scripts/ subdirectory, Dockerfile is in parent directory
    ...
    print_info "Build context: $(cd "$build_context" && pwd)"
...
show_help() {
    echo "FFmpeg Service - Harbor Registry Build Script"
    echo ""
    ...
    echo ""
    echo "Commands:"
    echo "  build         - Build FFmpeg service image with timestamp (for linux/amd64)"
    echo "  push          - Push latest FFmpeg service image to Harbor registry"
    echo "  build-push    - Build and push FFmpeg service image (recommended)"
    echo "  login         - Login to Harbor registry"
    echo "  platform      - Show current platform info"
    echo "  help          - Show this help message"
    echo ""
    echo "Services:"
    echo "  web           - FFmpeg service image (this project)"
    echo "  all           - Same as 'web'"
    echo ""
    echo "Examples:"
    echo "  $0 login                 # Login to Harbor registry"
    echo "  $0 build web             # Build FFmpeg service image"
    echo "  $0 build all             # Build (same as web)"
    echo "  $0 push web              # Push FFmpeg service image"
    echo "  $0 build-push all        # Build and push FFmpeg service image"
    echo ""
    echo "Image Format:"
    echo "  \$REGISTRY_DOMAIN_NAME/\$ENVIRONMENT/\$SERVICE:TIMESTAMP"
    echo "  Example: \$REGISTRY_DOMAIN_NAME/\$ENVIRONMENT/\$SERVICE:$(generate_timestamp)"
    echo ""
    echo "Notes:"
    ...
    echo "  - Build context is the parent directory (ffmpeg/)"
    echo ""
    echo "Credentials:"
    echo "  - Set REGISTRY_DOMAIN_NAME, HARBOR_USERNAME and HARBOR_PASSWORD env vars to use automatically"
    echo "  - Or script will prompt for registry domain, username, and password when needed"
    echo "  - Example: export REGISTRY_DOMAIN_NAME=example.harbor.io && export HARBOR_USERNAME=admin && export HARBOR_PASSWORD=secret"
    echo ""
}
...
case "$COMMAND" in
    ...
    push)
        ...
        ;;
    build-push)
        # Get registry, credentials first (prompt if needed)
        get_harbor_credentials
