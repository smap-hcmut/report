"""
Unit tests for TikTok TaskService handler batch upload integration.
...
class TestHandlerBatchIntegration:
    ...
    def mock_crawler_service(self):
        ...
        service.fetch_profile_videos = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_handle_fetch_profile_content_calls_batch_processing(
        self, task_service, mock_crawler_service, mock_crawl_result
    ):
        """
        Test that _handle_fetch_profile_content() calls batch processing.

        **Validates: Requirements 3.3**
        """
        # Setup mock return value
        mock_crawler_service.fetch_profile_videos.return_value = {
            "video_urls": ["url1", "url2"],
            "total": 2,
            "crawl_results": [mock_crawl_result, mock_crawl_result],
        }
        ...
        # Call handler
        payload = {"profile_url": "https://tiktok.com/@testuser"}
        result = await task_service._handle_fetch_profile_content(
