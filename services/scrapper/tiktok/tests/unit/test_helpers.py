"""
Unit tests for TikTok helper utilities.
...
class TestMapToNewFormat:
    ...
    def mock_content(self):
        ...
        content = MagicMock()
        content.external_id = "video123456"
        content.source = "tiktok"
        content.url = "https://tiktok.com/@user/video/123456"
        content.description = "Test video description"
        content.view_count = 10000
        content.like_count = 500
        content.comment_count = 50
        content.share_count = 25
        content.save_count = 10
        content.duration_seconds = 60
        content.tags = ["test", "video"]
        content.sound_name = "Original Sound"
    ...
    def test_meta_job_id_matches(self, mock_crawl_result):
        """Test that job_id in meta matches the provided job_id."""
    ...
    # Contract v2.0 Compliance Tests

    def test_platform_is_uppercase(self, mock_crawl_result):
        """Test that meta.platform is UPPERCASE per Contract v2.0."""
        result = map_to_new_format(
            result=mock_crawl_result,
            job_id="test-job-123",
            keyword="test keyword",
        ...
        assert result["meta"]["platform"] == "TIKTOK"
