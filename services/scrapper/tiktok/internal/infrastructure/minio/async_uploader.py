...
from .upload_task import UploadTask, UploadStatus, UploadResult, UploadState
...
class AsyncUploadManager:
    ...
    async def upload_async(
        ...
        # Initialize status
        status = UploadStatus(
            task_id=task_id, status=UploadState.QUEUED, total_bytes=len(data)
        )
    ...
    async def _process_task(self, task: UploadTask, worker_id: int) -> None:
        ...
        if status.status == UploadState.CANCELED:
            logger.info(f"[ASYNC_UPLOAD] Skipping canceled task {task_id}")
            return

        logger.info(
            f"[ASYNC_UPLOAD] Worker {worker_id} processing task {task_id}: "
            f"{task.get_size_mb():.2f} MB, bucket={task.bucket_name}, object={task.object_name}"
        )
        ...
        try:
            ...
            if task.enable_compression and task.compression_level > 0:
                ...
                logger.info(
                    f"[ASYNC_UPLOAD] Task {task_id}: Compressing (level={task.compression_level})"
                )

                compressed_data = await asyncio.to_thread(
                    self.compressor.compress, task.data, task.compression_level
                )
                ...
                logger.info(
                    f"[ASYNC_UPLOAD] Task {task_id} compressed: {compression_info.original_size} → "
                ...
                upload_metadata.update(
                    {
                        ...
                        "x-amz-meta-compressed-size": str(
            ...
            logger.info(
                f"[ASYNC_UPLOAD] Task {task_id}: Metadata prepared: {list(upload_metadata.keys())}"
            )

            # Step 3: Upload to MinIO
            status.status = UploadState.UPLOADING
            logger.info(f"[ASYNC_UPLOAD] Task {task_id}: Starting upload to MinIO...")

            # Ensure bucket exists
            logger.info(
                f"[ASYNC_UPLOAD] Task {task_id}: Ensuring bucket '{task.bucket_name}' exists..."
            )
            await asyncio.to_thread(self._ensure_bucket, task.bucket_name)
            logger.info(f"[ASYNC_UPLOAD] Task {task_id}: Bucket check complete")

            # Upload data
            logger.info(
                f"[ASYNC_UPLOAD] Task {task_id}: Calling put_object (size={len(data_to_upload)} bytes)..."
            )
            ...
            logger.info(f"[ASYNC_UPLOAD] Task {task_id}: put_object SUCCESS")
            ...
            result = UploadResult(
                ...
                compressed_size=(
                    compression_info.compressed_size
                    if compression_info
                    else len(data_to_upload)
                ),
                compression_ratio=(
                    compression_info.compression_ratio if compression_info else 1.0
                ),
                compression_algorithm=(
                    compression_info.algorithm if compression_info else None
                ),
            )
            self.results[task_id] = result

            logger.info(
                f"[ASYNC_UPLOAD] Task {task_id} completed successfully in "
                f"{result.duration:.2f}s, object_key={result.object_key}"
            )

        except Exception as e:
            logger.error(f"[ASYNC_UPLOAD] Task {task_id} FAILED: {e}", exc_info=True)
    ...
    def _ensure_bucket(self, bucket_name: str) -> None:
        ...
        """
        logger.info(f"[ASYNC_UPLOAD] Checking bucket '{bucket_name}' exists...")
        exists = self.minio_client.bucket_exists(bucket_name=bucket_name)
        logger.info(f"[ASYNC_UPLOAD] Bucket '{bucket_name}' exists: {exists}")
        if not exists:
            logger.info(f"[ASYNC_UPLOAD] Creating bucket '{bucket_name}'...")
            self.minio_client.make_bucket(bucket_name=bucket_name)
            logger.info(f"[ASYNC_UPLOAD] Bucket '{bucket_name}' created successfully")
