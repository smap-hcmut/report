...
This capability documents the HTTP server components that have been **REMOVED** from the Collector Service. The service now operates exclusively as a RabbitMQ consumer without any HTTP endpoints.

**Status:** DEPRECATED - All requirements below document removed functionality.

## Requirements

### Requirement: HTTP Server Removal

The Collector Service SHALL NOT run any HTTP server as it operates exclusively as a RabbitMQ consumer.

#### Scenario: No HTTP Server Running

- **WHEN** the Collector Service starts
- **THEN** no HTTP server SHALL be listening on any port
- **AND** only the RabbitMQ consumer SHALL be active

---

## Historical Requirements (REMOVED)
