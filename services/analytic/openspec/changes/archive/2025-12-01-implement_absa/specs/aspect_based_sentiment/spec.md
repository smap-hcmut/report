...
The Analytics Engine SHALL provide aspect-based sentiment analysis (ABSA) that determines sentiment for both overall text and individual business aspects (DESIGN, PERFORMANCE, PRICE, SERVICE, GENERAL) using the PhoBERT ONNX model with Context Windowing technique.

**Rationale**: Business users need actionable insights like "Customers like DESIGN but complain about PRICE" rather than just overall sentiment. ABSA enables aspect-level decision making.
