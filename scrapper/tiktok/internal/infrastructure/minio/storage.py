...
from application.interfaces import IArchiveStorage
...
class MinioMediaStorage(IArchiveStorage):
    """
    ...
    async def ensure_bucket(self) -> None:
        ...
        logger.debug(
            f"[MINIO] ensure_bucket called, bucket={self._bucket}, already_checked={self._bucket_checked}"
        )
        ...
        async with self._bucket_lock:
            ...
            logger.info(f"[MINIO] Checking if bucket '{self._bucket}' exists...")
            exists = await asyncio.to_thread(
                self._client.bucket_exists, bucket_name=self._bucket
            )
            logger.info(f"[MINIO] Bucket '{self._bucket}' exists: {exists}")
            if not exists:
                logger.info(f"[MINIO] Creating bucket '{self._bucket}'...")
                await asyncio.to_thread(
                    self._client.make_bucket, bucket_name=self._bucket
                )
                logger.info(f"[MINIO] Bucket '{self._bucket}' created successfully")
    ...
    async def upload_file(
        ...
        """
        logger.info(
            f"[MINIO] upload_file called: source={source}, object_name={object_name}, prefix={prefix}"
        )
        await self.ensure_bucket()

        key = self._build_object_name(object_name, prefix)
        logger.info(f"[MINIO] Built object key: {key}")
        ...
        logger.info(
            f"[MINIO] Uploading file to bucket={self._bucket}, key={key}, content_type={content_type}"
        )
        try:
            await asyncio.to_thread(
            ...
            logger.info(f"[MINIO] File upload successful: {key}")
        except Exception as e:
            logger.error(
                f"[MINIO] File upload FAILED: {key}, error: {e}", exc_info=True
            )
            raise
    ...
    async def upload_bytes(
        ...
        """
        logger.info(
            f"[MINIO] upload_bytes called: object_name={object_name}, prefix={prefix}, data_size={len(data)} bytes"
        )
        await self.ensure_bucket()
        ...
        compress = (
            ...
            else self._enable_compression
        )
        level = (
            compression_level
            if compression_level is not None
            else self._compression_level
        )
        logger.info(
            f"[MINIO] Built object key: {key}, compression={compress}, level={level}"
        )
        ...
        if compress and level > 0:
            logger.info(f"[MINIO] Compressing data (level={level})...")
            compressed = await asyncio.to_thread(self._compressor.compress, data, level)

            compression_info = self._compressor.get_compression_info(
                original_size=len(data), compressed_size=len(compressed), level=level
            )
            ...
            logger.info(
                f"[MINIO] Compressed {len(data)} → {len(compressed)} bytes "
                f"({compression_info.get_savings_percentage():.1f}% saved)"
            )

        # Upload
        logger.info(
            f"[MINIO] Starting put_object: bucket={self._bucket}, key={key}, size={len(data_to_upload)} bytes"
        )
        ...
        try:
            ...
            logger.info(f"[MINIO] put_object SUCCESS: {key}")
        except Exception as e:
            logger.error(f"[MINIO] put_object FAILED: {key}, error: {e}", exc_info=True)
            raise

        logger.info(f"[MINIO] Uploaded {len(data_to_upload)} bytes to {key}")
        return self.object_uri(key)  # Return MinIO URI instead of just key
    ...
    async def download_bytes(
        ...
        response = await asyncio.to_thread(
            self._client.get_object, bucket_name=self._bucket, object_name=key
        )
        ...
        if auto_decompress and metadata.get("x-amz-meta-compressed") == "true":
            ...
            data = await asyncio.to_thread(self._compressor.decompress, data)
            logger.debug(f"Decompressed to {len(data)} bytes")

        return data

    async def get_object_metadata(
        self, object_name: str, prefix: Optional[str] = None
    ) -> Dict[str, str]:
        """
        Get object metadata.
    ...
    async def get_presigned_url(
        self, object_uri: str, expires_hours: int = 168
    ) -> Optional[str]:
        """
