# YouTube Worker - Implementation Plan: Payload-Based Configuration

## Overview

Move configuration options from settings to payload parameters, allowing per-message control of:
- `media_download_enabled` - Enable/disable media file downloads
...
**File**: `scrapper/youtube/domain/entities/crawl_result.py`
...
**File**: `scrapper/youtube/application/interfaces.py`
...
**2.1.4** Update `_handle_fetch_channel_content()` method:
...
async def fetch_videos_batch(
    ...
    Args:
        ...
        include_channel: Whether to fetch channel info
...
async def _fetch_single_video(
    ...
    try:
        ...
        minio_key = await self.content_archiver.archive_content(
            content=content,
            author=author if include_channel else None,
...
**2.2.4** Update `fetch_channel_videos()` method:
...
**File**: `scrapper/youtube/internal/adapters/content_archiver.py`
...
class Settings(BaseSettings):
    ...
    enable_db_persistence: bool = True  # DEFAULT: Save Content/Author/Comment to MongoDB (can be overridden per-message)
...
#### 5.1 Unit Tests
**File**: `scrapper/youtube/tests/unit/test_task_service_payload_options.py`
...
async def test_crawl_links_with_media_disabled():
    ...
    # Prepare payload
    payload = {
        'video_urls': ['https://youtube.com/watch?v=test123'],
...
@pytest.mark.asyncio
async def test_research_and_crawl_with_mixed_options():
    """Test research_and_crawl with different option combinations"""
    # Setup mocks
    crawler_service = AsyncMock()
    job_repo = AsyncMock()
    content_repo = AsyncMock()
    ...
    task_service = TaskService(
        ...
        job_repo=job_repo,
        content_repo=content_repo,
        search_session_repo=search_session_repo,
    )
    ...
    job_repo.create_job.return_value = True
    job_repo.update_job_status.return_value = True
    crawler_service.search_and_fetch.return_value = {
        'video_urls': [],
        'total': 0,
        'successful': 0,
        'failed': 0,
    ...
    # Verify
    assert crawler_service.search_and_fetch.call_count == 2  # Two keywords
    for call in crawler_service.search_and_fetch.call_args_list:
        call_kwargs = call.kwargs
        assert call_kwargs['media_download_enabled'] == True
        assert call_kwargs['save_to_db_enabled'] == False
```

#### 5.2 Integration Tests
**File**: `scrapper/youtube/tests/integration/test_payload_options_integration.py`
...
async def test_end_to_end_with_db_disabled(test_db, test_minio):
    ...
    # Check MinIO
    minio_keys = await test_minio.list_objects('youtube-archive')
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_end_to_end_with_both_enabled(test_db, test_minio):
    """Test full flow with both MinIO and MongoDB enabled"""
    # Setup real services
    task_service = create_task_service(
        db=test_db,
        minio=test_minio,
    ...
    # Verify: Both MinIO and MongoDB have data
    assert result['success'] == True
...
#### 6.1 Update README
**File**: `scrapper/youtube/README.md`
...
This allows you to:
- Save only to MinIO archive (fast, compressed storage)
- Save only to MongoDB (queryable, structured storage)
- Save to both (full redundancy)
...
4. **Error Handling**: Catch specific exceptions, log with context, include full tracebacks
...
9. **Error Logging**: Use logger.error with exc_info=True for full tracebacks
...
5. **Edge Cases**: Test with missing fields, invalid values, boundary conditions
...
- YouTube worker defaults to `save_to_db_enabled: true` (current behavior)
...
## Special Considerations for YouTube Worker

### FFmpeg Service Integration
- Media downloads may involve FFmpeg service for audio extraction
- `media_download_enabled: false` will skip FFmpeg calls entirely
- Reduces load on FFmpeg service when only metadata is needed

### STT Integration
- STT transcription depends on media files being available
- If `media_download_enabled: false`, STT will be skipped
- Consider enabling media downloads if transcripts are needed

### Gemini AI Summaries
- AI summaries depend on transcripts
- If transcripts are unavailable, summaries will be skipped
- Enable both media downloads and STT for full AI summary support

### MongoDB vs MinIO Trade-offs
- **MongoDB**: Queryable, structured, good for analytics
- **MinIO**: Compressed, cheaper storage, good for archival
- **Both**: Full redundancy, best for production
- **MinIO only**: Fast, cheap, good for bulk scraping
