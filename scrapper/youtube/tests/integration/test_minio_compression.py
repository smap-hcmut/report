...
# Skip if MinIO not available - mark as integration test
...
class TestMinIOAsyncUpload:
    ...
    @pytest.mark.asyncio
    async def test_async_upload_progress_tracking(self, async_storage, sample_json_data):
        ...
        assert (
            UploadState.COMPRESSING in seen_states or
            UploadState.UPLOADING in seen_states
        )
    ...
    @pytest.mark.asyncio
    async def test_async_upload_with_custom_metadata(self, async_storage, sample_json_data):
