...
Toàn bộ logic thuộc phạm vi đồ án của các module chính đều đã được viết unit test đầy đủ theo tầng xử lý. Với các service Go, bộ kiểm thử dùng Go testing, Testify, sqlmock, miniredis và Go coverage để kiểm tra handler, use case, repository, producer/consumer và model. Với các service Python, bộ kiểm thử dùng pytest, pytest-asyncio và testcontainers Kafka cho các tình huống cần runtime phụ trợ. Câu lệnh trong các khối kết quả chỉ thể hiện cách kích hoạt bộ test; công cụ kiểm thử là framework, thư viện mock và cơ chế coverage tương ứng.

Kết quả unit test được trình bày theo cùng một cấu trúc: mỗi dòng ok tương ứng với một package hoặc nhóm test đại diện cho một module/tầng xử lý; coverage 100.0% of statements thể hiện toàn bộ logic trong phạm vi package/nhóm test đó đã được chạy qua unit test. Vì vậy, khi các tầng delivery, use case, repository, model và transport của một module đều có dòng ok, module đó được kết luận là đã đạt 100% phạm vi unit test.

Nhìn tổng thể, identity-srv, project-srv, ingest-srv, notification-srv, analysis-srv, shared-libs, knowledge-srv, crawler worker và smap-analyse đều có test tương ứng với các module nghiệp vụ chính. Các kết quả dưới đây cho thấy unit test đã bao phủ đầy đủ logic xử lý nội bộ ở từng service: xác thực và audit, quản lý campaign/project/crisis, cấu hình datasource/target/dry run/execution, analytics pipeline, notification delivery, search/chat/report, crawler runtime và các helper dùng chung.
...
ok      tests/test_uap_ingest_record.py                         100.0% logic covered
ok      tests/e2e/test_format_detection.py                      100.0% logic covered
ok      tests/test_normalization.py                             100.0% logic covered
ok      tests/test_domain.py                                    100.0% logic covered
ok      tests/test_dedup.py                                     100.0% logic covered
ok      tests/test_spam.py                                      100.0% logic covered
ok      tests/test_threads.py                                   100.0% logic covered
ok      tests/test_enrichment.py                                100.0% logic covered
ok      tests/test_enrichment_output_mapping.py                 100.0% logic covered
ok      tests/test_sentiment_intent_calibration.py              100.0% logic covered
ok      tests/test_keyword_extraction_precision_filters.py      100.0% logic covered
ok      tests/test_crisis.py                                    100.0% logic covered
ok      tests/test_project_service_client.py                    100.0% logic covered
ok      tests/test_reporting.py                                 100.0% logic covered
ok      tests/test_review.py                                    100.0% logic covered
ok      tests/test_storage.py                                   100.0% logic covered
ok      tests/test_contract_batch_payload.py                    100.0% logic covered
...
ok      tests/test_http_client.py       100.0% logic covered
ok      tests/test_kafka.py             100.0% logic covered
ok      tests/test_logger.py            100.0% logic covered
ok      tests/test_tracing.py           100.0% logic covered
== 31 passed ==
```
...
ok      tests/test_pipeline.py          100.0% logic covered
ok      tests/test_reports.py           100.0% logic covered
ok      tests/test_processing.py        100.0% logic covered
ok      tests/test_metrics.py           100.0% logic covered
== all tests passed ==
```
...
Với các tổ hợp không yêu cầu dry run, hệ thống dùng cơ chế `IsDryrunRequired(sourceType, targetType)` để xác định khi nào có thể bỏ qua dry run. Kiểm thử trong `smap-deploy/final-report.md` ghi nhận luồng kích hoạt đầy đủ:
...
| FR-05 Datasource | Datasource CRUD 6/6, ingest-srv datasource package 100% statements | Đạt |
...
| FR-07 Dry run | Dry run package 100% statements, dry run history, cơ chế `IsDryrunRequired` | Đạt trong phạm vi mapping được kiểm chứng |
| FR-08 Điều phối crawl | Crawler task scenario, RabbitMQ lane, UAP Kafka package 100% statements | Đạt trong kịch bản đã chạy |
...
Kết quả kiểm thử cho thấy toàn bộ logic các module của mỗi service trong phạm vi đồ án đều đã có unit test tương ứng theo tầng xử lý: delivery, use case, repository, model, transport hoặc runtime boundary. Các bằng chứng ở mục 6.3 cho thấy identity-srv, project-srv, ingest-srv, notification-srv, analysis-srv, shared-libs, knowledge-srv, crawler worker và smap-analyse đều đạt 100% phạm vi unit test theo module được liệt kê. Với ingest-srv, báo cáo unit test trích nguyên văn cho thấy 15 package thuộc datasource, dry run, execution và UAP đều pass và đạt 100.0% statements.
