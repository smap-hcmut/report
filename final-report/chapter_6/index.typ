...
Luồng thiết lập campaign/project, datasource, target và dry run được kiểm tra bằng API trực tiếp và bằng giao diện production.
...
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(inset: (y: 0.6em))[Target setup],
    table.cell(inset: (y: 0.6em))[Gọi tạo keyword, profile, post target; sau đó list, detail, update, deactivate, delete],
...
Kết luận UC-01: API thiết lập campaign/project, datasource, target và dry run đạt trong phạm vi đã kiểm thử. Giao diện hiển thị được dữ liệu thật và cập nhật đúng context campaign.

=== 6.4.4 UC-02: Vận hành chiến dịch theo dõi

Pha vận hành kiểm tra readiness, kích hoạt project và các thao tác lifecycle chính sau khi dữ liệu thiết lập đã tồn tại.
...
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(inset: (y: 0.6em))[Gọi readiness sau khi có đủ datasource và target hợp lệ],
    table.cell(inset: (y: 0.6em))[`can_proceed=true`, `missing_dryrun=0` trong kịch bản đã chạy],
    table.cell(inset: (y: 0.6em))[Project activation],
    table.cell(inset: (y: 0.6em))[Gọi `POST /projects/:id/activate` khi điều kiện sẵn sàng đạt],
    table.cell(inset: (y: 0.6em))[Thành công, datasource chuyển sang trạng thái vận hành tương ứng],
...
Kịch bản kích hoạt đầy đủ dùng dữ liệu đã thiết lập ở UC-01 làm tiền điều kiện. Với các tổ hợp không yêu cầu dry run, hệ thống dùng cơ chế `IsDryrunRequired(sourceType, targetType)` để xác định khi nào có thể bỏ qua dry run trước khi cho phép project đi vào vận hành. Một lần chạy ghi nhận luồng sau:
...
#block(width: 100%)[
  ...
  #table(
    ...
    stroke: 0.5pt,
    align: (left, left),
    table.cell(align: center + horizon, inset: (y: 0.6em))[*Bước kiểm tra readiness/activation*],
    table.cell(align: center + horizon, inset: (y: 0.6em))[*Kết quả thực tế*],
    table.cell(inset: (y: 0.6em))[Tiền điều kiện: campaign],
    table.cell(inset: (y: 0.6em))[Thành công, campaign ID `54cf8af8-...`],
    table.cell(inset: (y: 0.6em))[Tiền điều kiện: project],
    table.cell(inset: (y: 0.6em))[Thành công, project ID `14d8a0ab-...`],
    table.cell(inset: (y: 0.6em))[Tiền điều kiện: datasource],
    table.cell(inset: (y: 0.6em))[Thành công, datasource ID `0acb2ab6-...`, trạng thái `PENDING`],
    table.cell(inset: (y: 0.6em))[Tiền điều kiện: target],
    table.cell(inset: (y: 0.6em))[Thành công, target ID `8c4502e4-...`],
    table.cell(inset: (y: 0.6em))[Tiền điều kiện: target sẵn sàng],
...
Kết luận UC-02: readiness, project activation và lifecycle state change đã được kiểm chứng bằng API thật; các bước datasource/target/dry run chỉ đóng vai trò tiền điều kiện thiết lập từ UC-01.

=== 6.4.5 UC-03: Tra cứu và hỏi đáp dữ liệu phân tích

Knowledge-srv được kiểm tra ở các nhóm search, chat, conversation, suggestions và index statistics. Đây là phạm vi cốt lõi của UC-03 theo Chương 4.
...
Ngoài phạm vi UC-03 cốt lõi, Knowledge-srv còn có capability mở rộng cho report generation. Capability này được ghi nhận riêng để không làm lệch mapping use case đã định nghĩa ở Chương 4.
...
#block(width: 100%)[
  ...
  #table(
    ...
    stroke: 0.5pt,
    align: (left, left, left),
    table.cell(align: center + horizon, inset: (y: 0.6em))[*Capability mở rộng*],
    table.cell(align: center + horizon, inset: (y: 0.6em))[*Cách kiểm tra*],
    table.cell(align: center + horizon, inset: (y: 0.6em))[*Kết quả thực tế*],
...
Kết luận UC-03: search/chat đạt ở mức API contract và xử lý trạng thái không có dữ liệu. Capability report generation được ghi nhận riêng ở lớp mở rộng. Chương này không kết luận chất lượng nội dung câu trả lời hoặc báo cáo khi dữ liệu đầy đủ, vì bộ kiểm thử hiện tại chưa có benchmark định lượng cho chất lượng LLM/report.
