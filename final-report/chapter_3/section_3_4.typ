...
Độ trễ thấp là một trong những ưu điểm lớn nhất của WebSocket. Vì kết nối được giữ mở, không có chi phí của TCP handshake và HTTP header cho mỗi message. Message có thể được gửi ngay khi có dữ liệu mà không cần polling liên tục. Trong nhiều trường hợp thực tế, WebSocket thường cho độ trễ thấp hơn đáng kể so với polling truyền thống, dù giá trị cụ thể vẫn phụ thuộc vào điều kiện mạng và hạ tầng triển khai.
...
Long Polling là kỹ thuật cũ hơn, mô phỏng thời gian thực bằng cách client gửi HTTP request và server giữ request mở cho đến khi có dữ liệu hoặc hết thời gian chờ. Khi response được gửi, client sẽ gửi request mới để tiếp tục chờ dữ liệu. So với WebSocket hoặc các cơ chế push chuyên biệt, Long Polling thường có độ trễ cao hơn và chi phí lớn hơn do phải duy trì chu kỳ request-response lặp lại. Tuy nhiên, nó hoạt động với hầu hết hạ tầng HTTP và không gặp nhiều vấn đề tương thích.
