...
Identity Service là dịch vụ chịu trách nhiệm cho security boundary của toàn hệ thống SMAP. Dịch vụ này tập trung vào xác thực người dùng, quản lý session, phát hành token và hỗ trợ internal token validation cho các service khác.
...
Service này đáp ứng trực tiếp FR-01 về User Authentication và liên quan trực tiếp đến UC-01 về Authenticate User.
...
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(align: center + horizon, inset: (y: 0.8em))[OAuth Handlers],
    table.cell(align: center + horizon, inset: (y: 0.8em))[Xử lý login, callback, logout và `/me` ở lớp HTTP delivery],
...
Luồng xử lý chính của Identity Service có thể nhìn qua hai flow quan trọng: OAuth2 login flow và protected-route validation flow.
