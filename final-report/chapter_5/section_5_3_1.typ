...
Service này đáp ứng trực tiếp FR-01 về User Authentication như một supporting concern của toàn hệ thống. Ngoài capability auth-facing, service còn có một consumer lane phục vụ audit logging và truy vết vận hành.
...
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Xử lý login, callback, logout, `/me` và các internal validation routes ở lớp HTTP delivery],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Điều phối OAuth2 flow, xử lý callback, hoàn tất phiên truy cập],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Phát hành và xác thực token dùng cho protected routes và internal validation],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Tạo, duy trì và thu hồi session hoặc blacklist state khi cần],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Tiêu thụ audit events từ Kafka, gom batch theo size hoặc timeout và flush vào persistence layer],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Lưu thông tin người dùng, audit log và dữ liệu identity liên quan],
...
#align(center)[
  #image("../images/chapter_5/seq-identity-oauth-login-flow.svg", width: 92%)
...
#align(center)[
  #image("../images/chapter_5/seq-identity-protected-route-validation-flow.svg", width: 88%)
...
#align(center)[
  #image("../images/chapter_5/seq-identity-audit-processing-flow.svg", width: 88%)
  #context (align(center)[_Hình #image_counter.display(): Luồng audit event processing trong Identity Service_])
  #image_counter.step()
]
