...
==== 5.4.1.5 Redis TTL và data retention

Redis trong SMAP được dùng cho dữ liệu tạm thời hoặc có thể tái tạo từ nguồn bền vững, nên mỗi nhóm key có retention khác nhau theo vai trò vận hành:

- Session runtime của identity-srv dùng key `session:{jti}` với TTL mặc định theo `session.ttl` là 8 giờ; trường hợp remember-me dùng TTL 7 ngày. Mapping `user_sessions:{user_id}` được giữ 7 ngày để hỗ trợ logout-all và tự lọc các JTI đã hết hạn.
- Token blacklist dùng key `blacklist:{jti}` với TTL bằng thời gian sống còn lại của JWT, nhờ đó token đã bị revoke tự biến mất sau khi token gốc hết hạn.
- Domain registry dùng key `smap:domains` không đặt TTL vì đây là registry dùng chung do analysis-srv publish lại khi khởi động; nếu Redis mất dữ liệu, registry được tái tạo từ cấu hình domain khi service publish lại.
- Knowledge cache dùng TTL ngắn cho dữ liệu phụ thuộc truy vấn: campaign-name và campaign-project cache khoảng 10 phút, search result cache khoảng 5 phút. Embedding cache dùng TTL dài hơn, mặc định 7 ngày, vì vector theo nội dung đầu vào có khả năng tái sử dụng cao.
- Notification Redis Pub/Sub không dùng Redis làm durable message store; message chỉ được fanout tới WebSocket runtime tại thời điểm publish. Riêng key chống trùng crisis alert dùng TTL theo `repeat_cooldown`, mặc định 1 giờ nếu payload không cung cấp giá trị hợp lệ.

Vì vậy, mất dữ liệu Redis không được xem là mất dữ liệu nghiệp vụ bền vững. Các dữ liệu cần audit, lineage, report tracking hoặc search index vẫn được giữ ở PostgreSQL, MinIO hoặc Qdrant; Redis chỉ giữ session/cache/registry/pub-sub state có thể hết hạn hoặc tái tạo.
...
Analytics Service lưu kết quả phân tích và dữ liệu điều phối lane bất đồng bộ trong schema `schema_analysis`. Ở lớp relational, ba đối tượng chính là `post_insight`, `analytics_outbox` và `analytics_run_manifest`. Chúng phản ánh ba nhu cầu khác nhau: lưu insight đã enrich, đảm bảo publish downstream đáng tin cậy, và giữ run-level manifest cho audit hoặc replay context.
...
`post_insight` là bảng trung tâm của analytics schema. So với phương án tách `post_analytics` và `post_comments` thành các bảng chi tiết riêng, service dùng một cấu trúc insight giàu thông tin hơn, chứa nội dung, sentiment, aspects, keywords, risk, engagement, quality và processing metadata trong cùng một row. Điều này phù hợp với cách pipeline phân tích phát sinh `NLPFact` và persist thành `post_insight`.
...
- Direct Insight Persistence: kết quả enrichment được materialize trực tiếp thành `post_insight` thay vì chia nhỏ thành nhiều bảng chi tiết phụ.
...
Những lựa chọn này giúp hệ thống giữ được ranh giới ownership rõ ràng, hỗ trợ tracing cho các lane bất đồng bộ, và tránh nhét raw artifact hoặc vector retrieval vào những storage model không phù hợp. Đây cũng là nền dữ liệu để các mục giao tiếp, triển khai và liên hệ yêu cầu ở các phần sau được trình bày nhất quán với cấu trúc hệ thống đã mô tả trong chương này.
