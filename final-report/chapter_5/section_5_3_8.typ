...
- Analytics API Gateway: Gọi `analysis-api` từ server-side route handlers để lấy dữ liệu analytics phục vụ dashboard, insight cards và chart.
...
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Tổ chức route, layout, provider tree, navigation, dashboard workspace, report pages, settings và các vùng UI chính],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Khởi tạo OAuth login, kiểm tra user hiện hành, lưu trạng thái auth và chuyển hướng khi session không hợp lệ],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Chuẩn hóa endpoint, gửi request kèm cookie, unwrap response envelope và proxy browser request sang backend gateway],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Quản lý cache dữ liệu server, auth state, notification state, selected campaign scope và long-running report jobs],
    ...
    table.cell(align: center + horizon, inset: (y: 0.8em))[Analytics API Gateway],
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Thực thi các analytics route server-side, proxy request sang `analysis-api` và chuyển JSON response thành dữ liệu phù hợp cho UI],
    table.cell(align: center + horizon, inset: (y: 0.8em))[Analytics query / dashboard dataset],
    table.cell(align: center + horizon, inset: (y: 0.8em))[Next.js API routes + analysis-api],

    table.cell(align: center + horizon, inset: (y: 0.8em))[Knowledge Assistant Surface],
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Gửi chat và suggestion request theo campaign scope, hiển thị markdown answer, citations và follow-up suggestions],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Điều phối list/detail/process/posts/comments của report workflow, lưu job state và polling tiến trình xử lý],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Khởi động Next.js standalone server, mở BrowserWindow và đóng gói web UI thành desktop application],
...
Frontend Application có sáu luồng xử lý chính: auth and session flow, service API consumption flow, analytics API flow, knowledge assistant flow, report workflow flow và notification presentation flow.
...
#align(center)[
  #image("../images/chapter_5/seq-frontend-auth-session-flow.svg", width: 95%)
  #context (
    align(center)[_Hình #image_counter.display(): Luồng xác thực và thiết lập session trong Frontend Application_]
  )
  #image_counter.step()
]
...
#align(center)[
  #image("../images/chapter_5/seq-frontend-proxy-api-flow.svg", width: 95%)
  #context (
    align(center)[_Hình #image_counter.display(): Luồng gọi API same-origin qua proxy trong Frontend Application_]
  )
  #image_counter.step()
]
...
===== c. Analytics API Flow

Luồng này phục vụ dashboard, chart builder và các hook analytics.

1. UI gọi các route như `/api/analytics/kpis`, `/api/analytics/heap`, `/api/analytics/posts`, `/api/analytics/project-stats` hoặc `/api/analytics/platforms`.
2. Route handler chạy server-side và dùng analysis client để gọi `analysis-api` qua service nội bộ hoặc URL cấu hình bằng `ANALYSIS_API_INTERNAL_URL`.
3. `analysis-api` trả dữ liệu JSON đã chuẩn hóa cho từng view như KPI, platform distribution, sentiment, keyword, post list, project stats hoặc heap space.
4. Các hook analytics nhận JSON đã chuẩn hóa và bind vào card, chart, heap space hoặc bảng dữ liệu.
5. Campaign filtering được truyền qua query parameter để dashboard chỉ lấy dữ liệu thuộc scope đang chọn.
...
- Server-Side Analytics Proxy: Browser gọi same-origin route `/api/analytics/*`, còn frontend server forward request tới `analysis-api` để giữ network boundary và timeout handling tập trung.
...
- Giữ analytics service access ở server-side route handlers nhằm không buộc browser gọi trực tiếp service nội bộ.
...
- Analytics route handlers và analysis client server-side.
...
- `analysis-api` cho các endpoint analytics dashboard dưới `/api/v1/analytics/*`.
