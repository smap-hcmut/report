...
Trong kiến trúc của SMAP, các sơ đồ tập trung làm rõ cách các thành phần trao đổi qua internal HTTP, RabbitMQ, Kafka và Redis Pub/Sub, cũng như cách metadata hoặc artifacts đi qua PostgreSQL, Redis, MinIO và Qdrant theo từng luồng xử lý. Các nhãn UC trong các tiểu mục bên dưới được dùng như mã tham chiếu của từng sơ đồ trong phần trình bày này.

Một số sơ đồ dưới đây là supporting technical sequence cho một use case người dùng. Các sơ đồ này không biến xử lý nền thành use case mới, mà chỉ làm rõ chuỗi phối hợp giữa các runtime lane để tạo ra kết quả mà người dùng quan sát trong use case tương ứng.
