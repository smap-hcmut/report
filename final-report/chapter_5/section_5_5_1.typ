...
#align(center)[
  #image("../images/chapter_5/seq-uc01-campaign-project-flow.svg", width: 70%)
]
...
- Nếu cần, giao diện gửi request tạo campaign mới đến Project Service; sau đó gửi request tạo project trong campaign đã chọn.

- Project Service kiểm tra dữ liệu đầu vào và tạo project gắn với campaign tương ứng.
...
#align(center)[
  #image("../images/chapter_5/seq-uc01-datasource-target-flow.svg", width: 67%)
]
#context (
  align(
    center,
  )[_Hình #image_counter.display(): Sequence Diagram UC-01 Part 2: Khai báo nguồn dữ liệu và mục tiêu thu thập_]
)
#image_counter.step()
...
- Ingest Service lưu datasource, tiếp nhận các target được khai báo và cập nhật trạng thái cấu hình phục vụ bước readiness ở phía sau.
...
#align(center)[
  #image("../images/chapter_5/seq-uc01-dryrun-readiness-flow.svg", width: 81%)
]
#context (
  align(center)[_Hình #image_counter.display(): Sequence Diagram UC-01 Part 3: Kiểm tra thử và đánh giá mức sẵn sàng_]
)
#image_counter.step()
...
- Ingest Service nhận completion từ runtime thu thập, lưu kết quả kiểm tra thử gần nhất và readiness liên quan.
