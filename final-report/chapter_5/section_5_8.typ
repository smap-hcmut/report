...
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Tách bounded context theo identity, project, ingest, crawl execution, analytics, knowledge và notification; ownership dữ liệu và trách nhiệm được chia rõ theo service boundary],
    table.cell(align: horizon, inset: (y: 0.8em))[
      Mục 5.1.1, #linebreak()
      5.2.2, #linebreak()
      5.2.3, #linebreak()
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Tách lane đồng bộ và bất đồng bộ, tách pod theo vai trò API, consumer, scheduler và worker để hỗ trợ mở rộng theo workload],
    table.cell(align: horizon, inset: (y: 0.8em))[
      Mục 5.1.1, #linebreak()
      5.6.1, #linebreak()
      5.7.6
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Giữ request path gọn ở control plane, đẩy xử lý nặng sang các lane nền và chọn runtime phù hợp theo loại workload],
    table.cell(align: horizon, inset: (y: 0.8em))[
      Mục 5.1.2, #linebreak()
      5.2.4, #linebreak()
      5.6.1
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Tổ chức security boundary riêng ở `identity-srv`, dùng JWT, session, cookie và internal validation để bảo vệ các capability nghiệp vụ],
    table.cell(align: horizon, inset: (y: 0.8em))[
      Mục 5.2.3, #linebreak()
      5.3.1, #linebreak()
      5.3.8
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Dùng probe, tách workload theo lane và pod role, cùng các cơ chế khởi động lại hoặc triển khai lại phù hợp theo môi trường orchestration],
    table.cell(align: horizon, inset: (y: 0.8em))[
      Mục 5.6.4, #linebreak()
      5.7.6
    ],
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Duy trì khóa tương quan, lineage metadata, outbox hoặc tracking tables để nối task, artifact và dữ liệu downstream một cách nhất quán],
    table.cell(align: horizon, inset: (y: 0.8em))[
      Mục 5.4, #linebreak()
      5.6.2
    ],
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Tổ chức logging có ngữ cảnh, probe endpoints, trace ID propagation và các chỉ báo vận hành ở những runtime quan trọng],
    table.cell(align: horizon, inset: (y: 0.8em))[
      Mục 5.6.4, #linebreak()
      5.7.8
    ],
...
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Tách luồng xử lý nền khỏi request-response đồng bộ, dùng queue hoặc stream cho các lane analytics, ingest và indexing],
    table.cell(align: horizon, inset: (y: 0.8em))[
      Mục 5.1.2, #linebreak()
      5.6.1, #linebreak()
      5.6.2
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Dùng OAuth2, JWT, HttpOnly cookie, kiểm tra token và cơ chế xác thực nội bộ phù hợp với từng nhóm giao tiếp],
    table.cell(align: horizon, inset: (y: 0.8em))[
      Mục 5.3.1, #linebreak()
      5.3.8
    ],
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Tách workload theo vai trò, bổ sung probe và cơ chế triển khai lại để hỗ trợ khôi phục khi lane xử lý gặp lỗi],
    table.cell(align: horizon, inset: (y: 0.8em))[
      Mục 5.6.4, #linebreak()
      5.7.6
    ],
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Tách pod theo vai trò API, consumer, scheduler và worker để hỗ trợ mở rộng theo lane thay vì mở rộng đồng loạt toàn hệ thống],
    table.cell(align: horizon, inset: (y: 0.8em))[
      Mục 5.2.4, #linebreak()
      5.7.6
    ],
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Duy trì hợp đồng dữ liệu rõ ràng, tracking metadata, correlation key và raw artifact lineage giữa các thành phần bất đồng bộ],
    table.cell(align: horizon, inset: (y: 0.8em))[
      Mục 5.4, #linebreak()
      5.6.2
    ],
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Tách service boundary, ownership dữ liệu và delivery layer khỏi business logic để hỗ trợ thay đổi, bảo trì và kiểm thử],
    table.cell(align: horizon, inset: (y: 0.8em))[
      Mục 5.1.2, #linebreak()
      5.2.3, #linebreak()
      5.3
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Dùng logging có ngữ cảnh, trace ID propagation, health/readiness probes và chỉ báo vận hành cho các runtime quan trọng],
    table.cell(align: horizon, inset: (y: 0.8em))[
      Mục 5.6.4, #linebreak()
      5.7.8
    ],
...
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Không phải mọi runtime đều công bố cùng một surface metrics hoặc cùng một chuẩn observability],
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Dùng logging có ngữ cảnh, probe và metric instrumentation ở các lane quan trọng trước],
    table.cell(align: center + horizon, inset: (y: 0.8em))[P1],

    table.cell(align: horizon, inset: (y: 0.8em))[Distributed tracing visualization],
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Hệ thống có trace ID propagation nhưng chưa có lớp hiển thị trace trực quan xuyên service],
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Notification lane hiện tập trung vào WebSocket delivery và một số kênh ngoài trình duyệt như Discord; các kênh khác chưa phải capability chính],
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Knowledge-side report capability đã có nhưng workflow báo cáo và mẫu trình bày còn mang tính mở rộng, chưa phải trục năng lực cốt lõi của Chương 4],
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Analytics lane hiện thiên về mô hình và dữ liệu phục vụ tiếng Việt, chưa phải một pipeline đa ngôn ngữ đầy đủ],
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Giới hạn phạm vi phân tích theo miền và ngôn ngữ phù hợp với mô hình hiện hành],
    table.cell(align: center + horizon, inset: (y: 0.8em))[P2],
  )
]
...
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Khả năng crawl phụ thuộc vào chính sách, quota và thay đổi kỹ thuật từ các nền tảng bên ngoài],
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Dùng scheduling phù hợp, cache metadata, giảm tải không cần thiết và điều chỉnh lane crawl theo chính sách hiện hành],

    table.cell(align: horizon, inset: (y: 0.8em))[Độ ổn định của crawler],
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Các thay đổi ở giao diện hoặc contract của nền tảng bên ngoài có thể làm giảm độ ổn định của crawl runtime],
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Theo dõi lỗi runtime, tách worker pod và cho phép cập nhật lane crawl độc lập với control plane],

    table.cell(align: horizon, inset: (y: 0.8em))[Phạm vi ngôn ngữ của NLP],
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Các mô hình hiện hành được tối ưu theo phạm vi ngôn ngữ và miền dữ liệu nhất định],
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Giới hạn phạm vi phân tích, bổ sung human review ở các trường hợp nhạy cảm và mở rộng model khi cần],

    table.cell(align: horizon, inset: (y: 0.8em))[Độ trễ của cảnh báo],
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Realtime notification phụ thuộc vào cadence của ingest, analytics và notification lane; không phải mọi cảnh báo đều xuất hiện tức thời ở cấp mili-giây],
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Điều chỉnh chu kỳ xử lý, tách lane runtime và ưu tiên alert path khi workload yêu cầu],

    table.cell(align: horizon, inset: (y: 0.8em))[Tăng trưởng artifact và metadata],
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Raw artifact, report artifact và tracking metadata tăng theo thời gian vận hành và phạm vi campaign],
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Áp dụng retention policy, archive và lifecycle management phù hợp với môi trường triển khai],

    table.cell(align: horizon, inset: (y: 0.8em))[Độ phức tạp vận hành đa service],
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Nhiều runtime, nhiều storage và nhiều transport làm tăng chi phí vận hành so với kiến trúc đơn khối],
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Giữ ranh giới ownership rõ, chuẩn hóa contract giao tiếp và tách pod theo vai trò xử lý],
  )
]
...
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Task dispatch, analytics downstream và notification delivery cần tách khỏi request path để giảm coupling và giữ hệ thống linh hoạt hơn],
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[SMAP cần tách rõ business context, execution, analytics, retrieval và delivery theo workload specialization],
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[HTTP, RabbitMQ, Kafka, Redis Pub/Sub, PostgreSQL, MinIO và Qdrant được chọn để phù hợp hơn với từng loại dữ liệu và tương tác],
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Không phải mọi bước crawl, analytics hay indexing đều cần hoặc phù hợp với realtime end-to-end],
    ...
    table.cell(align: horizon, inset: (
      y: 0.8em,
    ))[Hệ thống cần nối được task, completion, raw artifact, indexing state và report artifact để phục vụ vận hành và truy vết],
  )
]
