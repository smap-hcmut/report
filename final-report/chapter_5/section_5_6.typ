...
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(align: center + horizon, inset: (y: 0.8em))[ingest-srv → scrapper-srv → ingest completion queues],
...
- scrapper-srv consume task, thực thi runtime và publish completion envelope trở lại;
...
- scrapper runtime materialize raw output vào local filesystem ở development mode hoặc MinIO ở production mode; ingest-srv chỉ nhận storage bucket, storage path, checksum, batch_id và completion metadata để tiếp tục xử lý;
...
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(align: center + horizon, inset: (y: 0.8em))[scrapper-srv qua tiktok_tasks],
    ...
    table.cell(align: center + horizon, inset: (y: 0.8em))[Direct exchange],
    table.cell(align: center + horizon, inset: (y: 0.8em))[ingest-srv],
    table.cell(align: center + horizon, inset: (y: 0.8em))[scrapper-srv qua facebook_tasks],
    ...
    table.cell(align: center + horizon, inset: (y: 0.8em))[Direct exchange],
    table.cell(align: center + horizon, inset: (y: 0.8em))[ingest-srv],
    table.cell(align: center + horizon, inset: (y: 0.8em))[scrapper-srv qua youtube_tasks],
...
- Completion envelope được scrapper-srv publish trực tiếp vào queue đích bằng queue name, thay vì đi qua một exchange tổng quát cho mọi completion event.
...
- `metric` và `threshold` để chỉ ra điều kiện bị vượt ngưỡng;
...
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[trace_id được gắn vào record; scrapper-srv còn intercept logging chuẩn của framework để gom về cùng logger],
  )
]
...
- Trong production mode, logger của scrapper-srv chuẩn hóa record thành các trường như timestamp, trace_id, level, caller, message và service.
...
Phần lớn Go API services trong SMAP đều expose bộ probe `health`, `ready` và `live`, nhưng nội dung kiểm tra cụ thể khác nhau theo dependency footprint của từng service. Ngược lại, Python runtime như scrapper-srv hiện chỉ expose một endpoint `health` đơn giản hơn.
...
- scrapper-srv trích xuất `X-Trace-Id` từ inbound HTTP requests và từ RabbitMQ message headers, sau đó tiếp tục dùng cùng trace_id khi publish completion envelope trở lại queue.
...
- RabbitMQ trong kiến trúc của SMAP chỉ đóng vai trò execution-plane broker giữa ingest-srv và scrapper-srv. Task được route theo exchange hoặc queue platform-specific, còn completion được correlate lại ở ingest lane bằng task_id và metadata kèm theo.
