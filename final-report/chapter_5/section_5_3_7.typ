...
Knowledge Service là dịch vụ chịu trách nhiệm cho knowledge retrieval và knowledge materialization layer của hệ thống SMAP. Khác với các service chỉ thuần HTTP hoặc chỉ thuần consumer runtime, service này kết hợp cả lane indexing downstream lẫn lane consumption đồng bộ để biến analytics outputs thành tri thức có thể tìm kiếm, hỏi đáp và tái sử dụng. Ngoài bộ capability cốt lõi đã trace về Chương 4, service còn có một nhánh report generation mở rộng ở phía knowledge layer.
...
- Extended Report Materialization Layer: Tổng hợp kết quả truy hồi, sinh các section bằng LLM và materialize artifact báo cáo.
- Knowledge Metadata Manager: Lưu conversations, messages, reports và indexing status phục vụ truy vết và tái sử dụng.

Service này đáp ứng trực tiếp FR-10 về Knowledge Search and Chat, đồng thời tham gia trực tiếp vào UC-05 về Execute Analytics and Build Knowledge và UC-06 về Search and Chat Over Knowledge. Capability report generation của service được xem như phần mở rộng ngoài bộ FR/UC cốt lõi hiện đã chốt ở Chương 4.
...
- Async Materialization Pattern: report generation được tách khỏi request path và materialize artifact xuống object storage ở capability mở rộng của knowledge layer.
...
- Giữ `Search UseCase` làm capability trung tâm được tái sử dụng bởi chat và bởi capability report mở rộng thay vì để từng domain tự truy hồi riêng.
