...
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(align: center + horizon, inset: (y: 0.8em))[Map platform sang queue, validate action và publish task payload cho các trigger cục bộ, hoặc trả kết quả kiểm tra khi được yêu cầu],
...
Scapper Worker Service có ba luồng đáng chú ý: auxiliary task submission flow, crawl execution and completion flow chính, và result inspection flow ở lớp hỗ trợ.

===== a. Auxiliary Task Submission Flow

Luồng này phản ánh bề mặt submit phụ trợ của service, hữu ích cho nhu cầu vận hành cục bộ, kiểm tra môi trường phát triển và một số trigger ngoài worker runtime. Đây không phải cơ chế handoff chính giữa ingest-srv và scapper-srv, vì luồng vận hành chính dùng request envelope với `task_id` do `ingest-srv` cấp.
...
#align(center)[
  ...
  #context (align(center)[_Hình #image_counter.display(): Luồng crawl execution và completion trong Scapper Worker Service_])
  #image_counter.step()
]
