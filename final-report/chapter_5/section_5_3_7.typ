...
- Report Generation Layer: Tổng hợp kết quả truy hồi, sinh các section bằng LLM và materialize artifact báo cáo.
- Knowledge Metadata Manager: Lưu conversations, messages, reports và indexing status phục vụ truy vết và tái sử dụng.

Service này đáp ứng trực tiếp FR-10 về Knowledge Search and Chat, đồng thời tham gia trực tiếp vào UC-05 về Execute Analytics and Build Knowledge và UC-06 về Search and Chat Over Knowledge.
...
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(align: center + horizon, inset: (y: 0.8em))[Report UseCase],
...
Knowledge Service có ba luồng xử lý chính: downstream indexing flow, contextual search and chat flow, và async report generation flow.

===== a. Downstream Indexing Flow

Luồng này bắt đầu khi dữ liệu analytics downstream được phát hành hoặc khi một internal indexing request được kích hoạt.

1. Consumer runtime nhận message từ các topic như `analytics.batch.completed`, `analytics.insights.published` hoặc `analytics.report.digest`, hoặc internal route nhận request index hoặc retry tương ứng.
2. Handler route message hoặc request sang usecase indexing phù hợp như `Index`, `IndexBatch`, `IndexInsight` hoặc `IndexDigest`.
...
===== b. Contextual Search and Chat Flow
...
===== c. Async Report Generation Flow

Luồng này bắt đầu khi client yêu cầu sinh một knowledge report.
...
- Internal Control Route Pattern: các route nội bộ cho index, retry, reconcile và statistics được tách biệt khỏi public knowledge APIs.
