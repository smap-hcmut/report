...
=== 5.3.7 Scrapper Worker Service

Scrapper Worker Service là runtime lane chịu trách nhiệm thực thi các crawl task bất đồng bộ trong execution plane của hệ thống SMAP. Service này tiếp nhận platform-specific task từ RabbitMQ, gọi TinLikeSub SDK để thực hiện các action crawl tương ứng, materialize raw artifact theo storage contract của runtime, rồi phát hành completion envelope trở lại ingest runtime để lane ingest tiếp tục xử lý downstream.

Khác với một worker thuần túy, service còn cung cấp một lớp FastAPI mỏng để submit task, kiểm tra kết quả và giám sát health của runtime khi cần cho nhu cầu vận hành cục bộ hoặc kiểm tra môi trường phát triển.

Vai trò của Scrapper Worker Service trong kiến trúc tổng thể:
...
- Auxiliary Task API Surface: Cung cấp các route submit hoặc kiểm tra kết quả để hỗ trợ thao tác vận hành cục bộ và kiểm tra runtime.
...
#align(center)[
  ...
  #context (align(center)[_Hình #image_counter.display(): Biểu đồ thành phần của Scrapper Worker Service_])
  #image_counter.step()
]

#context (align(center)[_Bảng #table_counter.display(): Thành phần chính của Scrapper Worker Service_])
#table_counter.step()
#block(width: 100%)[
  ...
  #table(
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Khởi động worker trong app lifespan, công bố router và health surface của runtime],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Map platform sang queue, validate action và publish task payload cho các trigger cục bộ, hoặc trả kết quả kiểm tra khi được yêu cầu],
    table.cell(align: center + horizon, inset: (y: 0.8em))[HTTP payload / RabbitMQ message or inspection response],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Giữ kết nối RabbitMQ, cấp một channel cho mỗi queue, consume message và điều phối vòng đời xử lý],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Duy trì mapping queue -> platform và action -> handler để route đúng crawl flow],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Thực thi các action như `search`, `post_detail`, `comments`, `video_detail` hoặc `full_flow` qua TinLikeSub SDK],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Materialize raw output xuống `output/` ở dev mode hoặc MinIO ở production mode, đồng thời sinh `batch_id` và `checksum`],
    ...
    table.cell(align: center + horizon, inset: (
      y: 0.8em,
    ))[Đóng gói completion envelope và route sang queue completion phù hợp cho execution hoặc dryrun lane],
...
Scrapper Worker Service có ba luồng đáng chú ý: auxiliary task submission flow, crawl execution and completion flow chính, và result inspection flow ở lớp hỗ trợ.

===== a. Auxiliary Task Submission Flow

Luồng này phản ánh bề mặt submit phụ trợ của service, hữu ích cho nhu cầu vận hành cục bộ, kiểm tra môi trường phát triển và một số trigger ngoài worker runtime. Đây không phải cơ chế handoff chính giữa ingest-srv và scrapper-srv, vì luồng vận hành chính dùng request envelope với `task_id` do `ingest-srv` cấp.
...
3. Service tạo `TaskPayload` với `task_id` và `created_at`, rồi publish message sang RabbitMQ queue tương ứng như một luồng gọi phụ trợ phục vụ kiểm tra hoặc vận hành cục bộ.
4. Message trở thành đầu vào cho worker runtime hoặc cho một worker instance khác đang consume cùng queue.

===== b. Crawl Execution and Completion Flow
...
#align(center)[
  ...
  #context (
    align(center)[_Hình #image_counter.display(): Luồng crawl execution và completion trong Scrapper Worker Service_]
  )
  #image_counter.step()
]

1. Worker runtime consume message từ queue platform-specific như `tiktok_tasks`, `facebook_tasks` hoặc `youtube_tasks`; trong luồng vận hành chính, message này tương ứng với request envelope chuẩn từ `ingest-srv`.
...
Luồng này chỉ đóng vai trò phụ trợ cho kiểm tra hoặc xác minh cục bộ.
...
3. Kết quả này hữu ích cho kiểm tra cục bộ, nhưng không thay thế cơ chế handoff chính thức của lane ingest vốn dựa trên raw artifact storage và completion envelope.

==== 5.3.7.3 Design Patterns áp dụng

Scrapper Worker Service áp dụng các design patterns sau:
...
- Duy trì submit API, local result API và `output/` artifacts như một bề mặt hỗ trợ kiểm tra, nhưng không xem đó là cơ chế handoff chính thức giữa ingest và worker runtime.
