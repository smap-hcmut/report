...
- Buffer per connection: channel `send` **buffered 256** (`make(chan []byte, 256)` — `usecase/new.go:52`)
...
| Connection buffer | "256 messages per connection" | Buffered 256 — `make(chan []byte, 256)` tại `usecase/new.go:52` |
