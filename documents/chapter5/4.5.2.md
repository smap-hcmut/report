...
Việc lựa chọn kiến trúc cho SMAP không chỉ là quyết định công nghệ, mà chính là quá trình giải quyết các mâu thuẫn kỹ thuật, đưa ra các quyết định kiến trúc (technical trade-offs) xuất phát từ đặc thù nghiệp vụ và yêu cầu phi chức năng của dự án. Trước khi phân tích từng phương án, nhóm xác định các "Architectural Drivers" chính (yếu tố thúc đẩy về kiến trúc) gồm:
...
- Đa dạng công nghệ (Polyglot Technology Stack):
  - Dự án yêu cầu Python cho Crawler, phân tích và AI (NLU/NLP – dùng PyTorch, Scikit-learn). 
  - Trong khi đó, Golang lại tối ưu cho API nhờ khả năng xử lý đa luồng tốt và tiết kiệm RAM.
...
Dựa trên các yếu tố này, nhóm đề bài đã lựa chọn 3 mô hình kiến trúc phổ biến để đánh giá và so sánh:
...
![Monolithic Architecture](images/monolithic_architecture.png)
...
![Modular Monolith Architecture](images/modular_monolith.png)
