...
Source tree hiện tại của `ingest-srv` cho thấy các module cốt lõi:
...
### 5.3.3 Tính năng 3: Project lifecycle control, datasource lifecycle và crawl mode update

#### Lớp lý thuyết

Trong các hệ thống crawl dữ liệu, lifecycle không chỉ tồn tại ở datasource mà còn ở lớp project business context. Các thao tác như activate, pause, resume hay thay đổi crawl mode thường cần kiểm tra điều kiện tiền đề, trạng thái hiện tại và ghi lại audit trail. Nếu bỏ qua các bước này, execution runtime sẽ trở nên khó kiểm soát và rất dễ mất tính nhất quán với business state.

#### Lớp phân tích trên dự án SMAP

Ở current-state, lifecycle control của SMAP chạy qua hai lớp liên kết. Ở lớp business control plane, `project-srv` nhận lệnh activate/pause/resume từ API public, gọi internal HTTP sang `ingest-srv` để kiểm tra readiness hoặc điều khiển runtime, rồi mới cập nhật `project.status` cục bộ. Sau khi transition thành công, `project-srv` publish lifecycle event Kafka như một lane lan truyền sự kiện hậu chuyển trạng thái. Ở lớp execution plane, `ingest-srv` hiện thực tiếp state machine cho project-level datasource runtime qua `project_lifecycle.go`, đồng thời vẫn có datasource-level lifecycle riêng qua `datasource_lifecycle.go`.

Ở cấp datasource, hàm `ActivateDataSource` chỉ cho phép chuyển từ `READY` sang `ACTIVE` khi runtime prerequisites đã thỏa. Hàm `PauseDataSource` chỉ cho phép pause khi source đang `ACTIVE`. Hàm `ResumeDataSource` chỉ cho phép resume khi source đang `PAUSED`. Đặc biệt, hàm `UpdateCrawlMode` không chỉ cập nhật mode mới mà còn ghi thêm một record vào `crawl_mode_changes`, nhờ đó hệ thống có audit trail cho các lần thay đổi chính sách crawl.

Ưu điểm của thiết kế này là các thao tác điều khiển quan trọng không bị dồn hết vào một transport duy nhất. Internal HTTP giữ được phản hồi đồng bộ cho readiness/lifecycle control giữa `project-srv` và `ingest-srv`, trong khi Kafka lifecycle event vẫn sẵn sàng cho propagation xuống các consumer khác. Đồng thời, datasource-level usecase vẫn bảo vệ execution plane khỏi các lệnh không hợp lệ và tăng khả năng truy vết khi cần kiểm tra lịch sử vận hành.

#### Lớp minh họa từ mã nguồn

Tính năng này được hiện thực tại `../project-srv/internal/project/usecase/lifecycle.go`, `../project-srv/pkg/microservice/ingest/usecase.go`, `../ingest-srv/internal/datasource/usecase/project_lifecycle.go` và `../ingest-srv/internal/datasource/usecase/datasource_lifecycle.go`.
...
if err := uc.ingest.Activate(ctx, current.ID); err != nil {
    return project.ActivateOutput{}, project.MapLifecycleClientError(err)
}
```
...
Theo nguyên tắc bằng chứng mã nguồn, phần giao diện người dùng chỉ nên mô tả các surface và công nghệ có bằng chứng trực tiếp trong workspace. Ở current-state, workspace đã chứa frontend source trong `../web-ui`, cùng `package.json`, `next.config.ts`, các `pages/`, `components/`, `hooks/` và các API service adapters. Vì vậy, có thể xác nhận sự tồn tại của một web client hiện thực bằng `Next.js` và `React`, nhưng vẫn nên tránh overclaim về toàn bộ kiến trúc frontend nếu chưa audit sâu toàn bộ flow UI.

### 5.5.2 Những gì có thể khẳng định

Từ mã nguồn hiện có, có thể xác nhận các interaction surfaces sau:

- Web client dùng `Next.js` và `React`, được xác nhận tại `../web-ui/package.json` và `../web-ui/next.config.ts`
- Report wizard UI theo project, được xác nhận tại `../web-ui/pages/projects/[project_id]/report-wizard.tsx`
- WebSocket client logic cho project/job notifications, được xác nhận tại `../web-ui/hooks/useProjectWebSocket.ts` và `../web-ui/services/websocketService.ts`
- Dashboard, trend-analysis, authentication và i18n resources, được thể hiện qua `../web-ui/pages/*` và `../web-ui/public/locales/*`

### 5.5.3 Kết luận cho phần UI gallery

Phần UI gallery vì vậy không còn là khu vực “ngoài phạm vi xác nhận trực tiếp”. Tuy nhiên, trong bản luận văn hiện tại, nó vẫn nên được trình bày ở mức có chọn lọc: chỉ đưa những page hoặc flow đã có evidence rõ trong source tree, thay vì cố gắng mô tả toàn bộ frontend như một subsystem đã được audit hoàn chỉnh ngang mức backend.
