...
Hạn chế thứ nhất nằm ở độ sâu audit giữa các lớp của workspace. Mã nguồn hiện tại đã chứa frontend source code trong `web-ui`, đủ để xác nhận framework giao diện và một số interaction surfaces chính. Tuy nhiên, luận văn hiện vẫn đi sâu hơn nhiều ở backend, runtime contracts và integration architecture so với phần frontend. Vì vậy, lớp giao diện mới được mô tả ở mức công nghệ và các flow tiêu biểu có bằng chứng rõ, chứ chưa được phân tích toàn diện ngang với các service backend.
