...
== 5.2 Lựa chọn Architecture Style

=== 5.2.1 Bối cảnh quyết định
...
====== 5.2.2.2. Modular Monolith (Nguyên khối mô-đun hóa)
...
====== 5.2.2.3. Microservices Architecture (Kiến trúc vi dịch vụ)
...
// #### 4.6.2.3 Ma trận quyết định (Weighted Decision Matrix)

// Sau khi phân tích chi tiết từng mô hình kiến trúc ở mục 4.6.2.2, cần một phương pháp định lượng để so sánh khách quan các lựa chọn. Ma trận quyết định được xây dựng dựa trên hai cấp độ đánh giá:
...
// Để lượng hóa sự lựa chọn phù hợp với context của SMAP, nhóm xây dựng ma trận trọng số tập trung vào **ba Architectural Drivers chính** đã được xác định tại mục 4.6.2.1, cùng với hai tiêu chí bổ sung để đánh giá toàn diện:
...
// - Chi phí vận hành & hạ tầng - 10%: Microservices đạt điểm 2, thấp hơn Monolith và Modular Monolith (5 điểm) do tốn chi phí DevOps và quản lý Kubernetes control plane. Tuy nhiên, như đã phân tích ở mục 4.6.2.2, chi phí này được bù đắp bởi khả năng scale chính xác, chỉ trả tiền cho tài nguyên thực sự sử dụng.

// **Kết luận:** Ma trận quyết định xác nhận rằng Microservices Architecture là lựa chọn tối ưu cho SMAP, đặc biệt mạnh ở ba tiêu chí chính (75% trọng số) phản ánh trực tiếp các Architectural Drivers đã được xác định ở mục 4.6.2.1. Các trade-offs ở hai tiêu chí bổ sung (25% trọng số) là chấp nhận được và đã được nhóm chuẩn bị sẵn các chiến lược giảm thiểu rủi ro (xem mục 4.6.2.5).

// #### 4.6.2.4 Kết luận và biện luận kiến trúc (Architecture Justification)

// Dựa trên kết quả từ ma trận quyết định (4.25/5.00) ở mục 4.6.2.3 và các phân tích kỹ thuật chi tiết ở mục 4.6.2.2, nhóm phát triển lựa chọn **Microservices Architecture** làm nền tảng cho SMAP. Quyết định này không chạy theo trào lưu hay "hype-driven development" mà là kết quả của một quá trình phân tích có phương pháp luận rõ ràng, nhằm đạt điểm cân bằng tối ưu giữa tính linh hoạt (Flexibility) và độ phức tạp (Complexity), đồng thời giải quyết triệt để các thách thức kỹ thuật đã được xác định ở mục 4.6.2.1.
...
// - Vấn đề: Analytics chạy AI inference nặng, dễ gặp OOM hoặc treo tiến trình. Như đã phân tích ở mục 4.6.2.2, module Analytics khi load model PhoBERT (~1.2GB) có thể gây memory leak, và trong Monolith/Modular Monolith, điều này sẽ làm sập toàn bộ ứng dụng.
...
// Ba luận điểm trên, cùng với phân tích định lượng, cho thấy Microservices Architecture không chỉ giải quyết được các thách thức kỹ thuật đã xác định ở mục 4.6.2.1 (Polyglot Runtime, Asymmetric Workload, Fault Isolation) mà còn mang lại các lợi ích đo lường được về mặt hiệu năng, chi phí và độ tin cậy.
...
// Tuy nhiên, như đã đề cập ở mục 4.6.2.2, việc áp dụng Microservices cũng mang lại những thách thức mới về độ phức tạp vận hành, độ trễ mạng và nhất quán dữ liệu. Nhóm hoàn toàn nhận thức rõ các thách thức này và đã chủ động chuẩn bị sẵn các chiến lược giảm thiểu rủi ro cụ thể, được trình bày chi tiết ở mục tiếp theo.

// ---

// #### 4.6.2.5 Phân tích Đánh đổi và Chiến lược giảm thiểu rủi ro (Trade-off Analysis)

// Như đã được đề cập ở phần kết luận mục 4.6.2.4, nhóm nhận thức rõ rằng **Microservices không phải là "viên đạn bạc" (Silver Bullet)**. Mặc dù kiến trúc này giải quyết triệt để ba Architectural Drivers chính của SMAP (Polyglot Runtime, Asymmetric Workload, Fault Isolation), nhưng nó cũng gia tăng đáng kể độ phức tạp hệ thống phân tán so với Monolithic Architecture.
...
//    - Kubernetes: Đã được tích hợp vào hạ tầng deployment với cấu hình HPA, resource limits, liveness/readiness probes (xem chi tiết ở mục 4.6.5).
...
//    - Event-Driven Architecture: RabbitMQ được triển khai với Outbox Pattern để đảm bảo reliable message delivery (xem mục 4.6.5.4 và 4.6.5.5).
...
// **Kết luận chương 4.6.2:**

// Thông qua một quá trình phân tích có phương pháp luận rõ ràng bao gồm:

// 1. **Xác định Architectural Drivers** (mục 4.6.2.1): Asymmetric Workload, Polyglot Technology Stack, Availability & Isolation.
// 2. **So sánh chi tiết các lựa chọn** (mục 4.6.2.2): Phân tích ưu/nhược điểm của Monolith, Modular Monolith và Microservices trong context cụ thể của SMAP.
// 3. **Đánh giá định lượng** (mục 4.6.2.3): Ma trận quyết định cho điểm 4.25/5.00 cho Microservices, vượt trội ở ba tiêu chí quan trọng nhất (75% trọng số).
// 4. **Biện luận kiến trúc** (mục 4.6.2.4): Ba luận điểm cốt lõi với số liệu định lượng cụ thể (giảm 96% image size, tiết kiệm 60-70% infrastructure cost, giảm 86% blast radius).
// 5. **Trade-off Analysis** (mục 4.6.2.5): Nhận diện đầy đủ các thách thức và chuẩn bị sẵn chiến lược giảm thiểu rủi ro với các công nghệ cụ thể: Kubernetes, Jenkins CI/CD, On-Premise Deployment, Claim-Check Pattern, RabbitMQ, Rancher, Prometheus, Grafana, OpenTelemetry, Jaeger, và ELK Stack.
