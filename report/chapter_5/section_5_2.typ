...
- Phát triển độc lập: Các team có thể phát triển và triển khai services độc lập, không cần chờ các team khác, tăng tốc độ phát triển và đưa sản phẩm ra thị trường nhanh hơn.
...
*b. Independent Scaling Evidence:*
...
- Polyglot Runtime: Là yêu cầu bắt buộc do đặc thù kỹ thuật. Python không thể thay thế cho AI/NLP, Golang không thể thay thế cho API performance. Monolith và Modular Monolith đều không giải quyết được vấn đề này.

- Independent Scaling: Chênh lệch workload lớn giữa Crawler/Analytics và API. Microservices cho phép scale chính xác từng service, tiết kiệm chi phí và đáp ứng yêu cầu về Scalability.

- Fault Isolation: Đảm bảo yêu cầu Availability. Analytics crash không làm sập Identity/Project, đáp ứng AC-1.

- Evidence từ code: Hệ thống đã triển khai thành công với 7 core services và 3 supporting services, chứng minh tính khả thi và hiệu quả của Microservices.
...
#block(width: 100%)[
  #set par(justify: false)
  #table(
    columns: (0.15fr, 0.15fr, 0.30fr, 0.20fr, 0.2fr),
    stroke: 0.5pt,
...
Container Diagram thể hiện 10 application containers, 7 core services và 3 supporting services và 5 data stores, PostgreSQL, MongoDB, Redis, MinIO, RabbitMQ, cùng với các communication patterns chính:
